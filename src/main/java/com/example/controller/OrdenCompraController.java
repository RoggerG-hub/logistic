...
public class OrdenCompraController {
	...
	public String registrarOrdenes(@Validated @ModelAttribute OrdenCompra orden, BindingResult result, Model model) {		
		if(result.hasErrors()) 
		{
			model.addAttribute("mensaje", "Ingresar el correlativo");
			model.addAttribute("ordenCompra", new OrdenCompra());
			model.addAttribute("proveedores",proveedorService.listaP());
			model.addAttribute("productos",productoService.listarProductos());
	...
	public String registrarDetalles(@Validated @ModelAttribute OrdenProducto ordenpp, BindingResult result, Model model) {	
	...
	/*@PostMapping("/ordenes/edit/post/{id}")
	public String updateLibro(@PathVariable Long id, @ModelAttribute("ordenCompraC") OrdenCompra ordenCompra, Model model) {
	...
	public String editCFF(@PathVariable Long id, Model model) {
		OrdenProducto od = new OrdenProducto();
		for(int i=0;i<listaOrdenesProductos.size();i++) 
		{
			if(listaOrdenesProductos.get(i).getId()==id) 
			{
